(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    var results = [];
    var iteratorResults = [];
    var uniqueIteratorResults = [];

    if (iterator) {
      _.each (array, function(element) {
        iteratorResults.push(iterator(element));
      });
      _.each (iteratorResults, function(element) {
        if (uniqueIteratorResults.indexOf(element) === -1) {
          uniqueIteratorResults.push(element);
        }
      });
      for ( var i = 0; i < uniqueIteratorResults.length; i++ ) {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function (item) {
      arrayOfMapped.push(iterator(item));
    }); 
    // map() is a useful primitive iteration function that works a lot
    // like each(), but in addition to running the operation on all
    // the members, it also maintains an array of results.
    return arrayOfMapped;
  };
